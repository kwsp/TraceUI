...
private slots:
    ...
    void testCircularOverwrite() {
        ...
        for (int i = 0; i < ScrollingHistoryProvider::kSize; ++i)
            provider.pushSample(static_cast<float>(i), static_cast<float>(i * 2), 100.0F);
    ...
    void testGraphScaleStaysAtOne() {
        TestableHistoryProvider provider;
        QSignalSpy scaleSpy(&provider, &ScrollingHistoryProvider::graphScaleChanged);
    ...
    void testNetworkProviderMultipleSamples() {
        NetworkHistoryProvider net;
        for (int i = 0; i < 20; ++i)
            net.onDataUpdated(static_cast<double>(i) * 100.0, static_cast<double>(i) * 50.0);
