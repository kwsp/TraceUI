        ...
        if (m_prevCpuInfo) {
            ...
            for (unsigned int i = 0; i < m_numCPUs; ++i) {
                integer_t inUse = (cpuInfo[(CPU_STATE_MAX * i) + CPU_STATE_USER]   - m_prevCpuInfo[(CPU_STATE_MAX * i) + CPU_STATE_USER])
                                ...
                                + (cpuInfo[(CPU_STATE_MAX * i) + CPU_STATE_NICE]   - m_prevCpuInfo[(CPU_STATE_MAX * i) + CPU_STATE_NICE]);
                integer_t total = inUse + (cpuInfo[(CPU_STATE_MAX * i) + CPU_STATE_IDLE] - m_prevCpuInfo[(CPU_STATE_MAX * i) + CPU_STATE_IDLE]);
                double percent = (total == 0) ? 0.0 : (double)inUse / total * 100.0;
                newUsages.append(percent);
            }
            m_cpuUsagePercent = newUsages;
