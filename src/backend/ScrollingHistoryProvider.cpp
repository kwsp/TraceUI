...
void ScrollingHistoryProvider::pushSample(float ch0, float ch1, float minMaxRef) {
    {
...
QSGNode *ScrollingHistoryProvider::updatePaintNode(QSGNode *oldNode,
...

    {
        ...
        for (int i = 0; i < kSize; i++) {
            ...
            m_img.setPixel(i, 0,
                           qRgba(static_cast<int>(n0 * kUint8Max), static_cast<int>(n1 * kUint8Max),
                                 0, kUint8Max));
        }
        m_dirty = false;
    }
    ...
    m_provider->m_texture = window()->createTextureFromImage(m_img, QQuickWindow::TextureIsOpaque);
