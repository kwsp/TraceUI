...
QVariant ProcessListModel::data(const QModelIndex& index, int role) const {
    ...
    case CpuPctRole: return e.cpuPct;
    case RamMBRole:  return e.ramMB;
    default:         return {};
    }
}
...
void ProcessListModel::updateData(QList<ProcessEntry> incoming) {
    ...
    for (int i = 0; i < common; ++i) {
        if (m_data[i].name   != incoming[i].name   ||
            m_data[i].pid    != incoming[i].pid    ||
            m_data[i].cpuPct != incoming[i].cpuPct ||
            m_data[i].ramMB  != incoming[i].ramMB) {
            m_data[i] = incoming[i];
